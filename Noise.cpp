...
float Noise::MakeNoise(Position3 pos)
{
	...
	float lerp1 = this->Lerp(
				  this->Lerp(this->Grad(aaa,Vector3(xf,yf,zf)), this->Grad(baa,Vector3(xf -1, yf, zf)), xfs),
				  this->Lerp(this->Grad(aba,Vector3(xf, yf - 1, zfs)), this->Grad(bba, Vector3(xf - 1, yf - 1, zf)), xfs)
							,yfs);
	
	float lerp2 = this->Lerp(
		this->Lerp(this->Grad(aab, Vector3(xf, yf, zf -1)), this->Grad(bab, Vector3(xf - 1, yf, zf -1)), xfs),
		this->Lerp(this->Grad(abb, Vector3(xf, yf -1, zf -1)), this->Grad(bbb, Vector3(xf - 1, yf-1, zf -1)),xfs)
							,yfs);

	return this->Lerp(zfs,lerp1,lerp2);
}
